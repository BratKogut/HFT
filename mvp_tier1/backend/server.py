...
async def lifespan(app: FastAPI):
    ...
    global market_data_handler, risk_manager, order_executor, active_strategies
    ...
    active_strategies = [momentum_strategy, mean_reversion_strategy]
    ...
    market_data_handler.register_ticker_callback(on_ticker)
    
    # Connect to exchange
    await market_data_handler.connect()
    
    # Start market data subscriptions
    asyncio.create_task(market_data_handler.subscribe_ticker(settings.trading_pair))
    ...
    await market_data_handler.disconnect()
...
async def handle_signal(signal):
    ...
    if signal.direction == 'long':
        ...
        if signal.symbol in risk_manager.positions:
            pnl = risk_manager.close_position(signal.symbol, signal.price)
            await order_executor.execute_order(
                symbol=signal.symbol,
                side='sell' if risk_manager.positions[signal.symbol].side == 'long' else 'buy',
                size=risk_manager.positions[signal.symbol].size
            )
    ...
    is_allowed, reason = risk_manager.pre_trade_check(
        symbol=signal.symbol,
...
async def health():
    ...
    return {
        "status": "healthy",
        "market_data": market_data_handler.is_running if market_data_handler else False,
...
async def get_stats():
    ...
    return {
        ...
        "execution": order_executor.get_stats() if order_executor else {},
        "strategies": [s.get_stats() for s in active_strategies]
    }
...
async def get_orders(limit: int = 100):
    ...
    if not order_executor:
        return []
    return order_executor.get_order_history(limit)
...
async def halt_trading():
    ...
    if risk_manager:
        risk_manager.halt_trading("Manual halt via API")
