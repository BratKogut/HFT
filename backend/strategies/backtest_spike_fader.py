"""
Backtest Volatility Spike Fader on real Binance data
"""
...
from volatility_spike_fader import VolatilitySpikeFader


async def backtest_spike_fader(data_file: str, initial_capital: float = 10000.0):
    """Run backtest on historical data"""
    
    print("="*80)
    print("⚡ VOLATILITY SPIKE FADER BACKTEST")
    print("="*80)
    print()
    ...
    # Initialize
    strategy = VolatilitySpikeFader(
        spike_threshold=0.012,          # 1.2% spike (realistic for BTC)
        min_spike_magnitude=0.010,      # Min 1.0%
        base_take_profit_pct=0.008,     # 0.8% TP
        base_stop_loss_pct=0.006        # 0.6% SL
    )
    ...
    for idx, row in df.iterrows():
        ...
        # If no position, look for entry
            ...
            signal = strategy.analyze_market(current_price, volume, timestamp)
            
            if signal:
                # Enter position
                position = signal
                print(f"\n⚡ SPIKE DETECTED: {signal['reason']}")
...
if __name__ == "__main__":
    ...
    if os.path.exists(data_file):
        results = asyncio.run(backtest_spike_fader(data_file, initial_capital=10000.0))
    else:
