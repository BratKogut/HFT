...
class OptimizedLiquidationHunter:
    ...
    def _detect_liquidation_setup(self, price, volume):
        ...
        # SHORT signal: Price at recent high + volume spike
        if price_position > 0.8 and volume_ratio > self.min_volume_ratio:
            signal = 'SHORT'
            # Confidence based on extremity and volume
            confidence = 0.5 + (price_position - 0.8) * 2.5  # 0.5-1.0
            confidence += min((volume_ratio - self.min_volume_ratio) * 0.1, 0.3)
            confidence = min(confidence, 1.0)
        
        # LONG signal: Price at recent low + volume spike
        elif price_position < 0.2 and volume_ratio > self.min_volume_ratio:
            signal = 'LONG'
            # Confidence based on extremity and volume
            confidence = 0.5 + (0.2 - price_position) * 2.5  # 0.5-1.0
            confidence += min((volume_ratio - self.min_volume_ratio) * 0.1, 0.3)
            confidence = min(confidence, 1.0)
